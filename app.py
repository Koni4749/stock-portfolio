...
if df is not None:
    ...
    with row1_col1:
        ...
        fig_donut.update_traces(textposition='inside', textinfo='percent+label', hovertemplate='%{label}<br>비중: %{percent:.2%}<extra></extra>')
        st.plotly_chart(fig_donut, use_container_width=True)

    with row1_col2:
        sector_df = df.groupby('섹터')['평가금액'].sum().reset_index()
        fig_pie = px.pie(sector_df, values='평가금액', names='섹터', title='업종별 투자 비중')
        fig_pie.update_traces(textposition='inside', textinfo='percent+label', hovertemplate='%{label}<br>비중: %{percent:.2%}<extra></extra>')
    ...
    y_padding = (y_max - y_min) * 0.25 if (y_max - y_min) != 0 else 10
    ...
    with tab2:
        ...
        fig_mdd_custom.add_trace(go.Scatter(x=drawdown_df.index, y=drawdown_df['내 포트폴리오']*100,
                                            ...
                                            hovertemplate='내 포트폴리오: %{y:.2f}%<extra></extra>'))
